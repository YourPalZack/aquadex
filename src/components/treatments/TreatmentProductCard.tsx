...
import type { WaterTreatmentProduct } from '@/types';
import { Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { ExternalLink, Info, FlaskConical, Tag, Trash2, Package } from 'lucide-react';
...
export default function TreatmentProductCard({ product, onDelete }: TreatmentProductCardProps) {
  return (
    <Card className="w-full shadow-lg flex flex-col h-full bg-card hover:shadow-xl transition-shadow duration-300">
      <CardHeader className="pb-4">
        <div className="flex justify-between items-start gap-3">
          ...
          <Button variant="ghost" size="icon" onClick={() => onDelete(product.id)} aria-label="Delete treatment product" className="text-destructive hover:bg-destructive/10">
            <Trash2 className="w-4 h-4" />
          </Button>
        </div>
        {product.brand && (
          <CardDescription className="flex items-center">
            <Tag className="w-4 h-4 mr-1.5 text-muted-foreground" /> Brand: {product.brand}
          </CardDescription>
        )}
        {product.type && (
          <CardDescription className="mt-1 flex items-center">
             <Package className="w-4 h-4 mr-1.5 text-muted-foreground" /> Type: {product.type}
          </CardDescription>
        )}
      </CardHeader>
            ...
            <ul className="space-y-2">
              {product.amazonLinks.map((link, index) => (
                <li key={index}>
                  ...
                  >
                    <a href={link.url} target="_blank" rel="noopener noreferrer" title={`Search for ${product.name} on ${link.storeName}`}>
                      <Image 
