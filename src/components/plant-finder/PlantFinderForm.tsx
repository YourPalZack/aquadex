...
import { findPlantAction } from '@/lib/actions';

const initialState: FindPlantActionState = {
  message: null,
  errors: null,
  searchResults: null,
  aiMessage: null,
...
interface PlantFinderFormProps {
  onSearchComplete: (results: FindPlantActionState) => void;
}
...
export default function PlantFinderForm({ onSearchComplete }: PlantFinderFormProps) {
  const [state, formAction] = useActionState(findPlantAction, initialState);
    ...
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="flex items-center">
            ...
            Aquatic Plant Search
        </CardTitle>
        <CardDescription>Enter the name of the aquatic plant you're looking for.</CardDescription>
      </CardHeader>
      <form action={formAction}>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="speciesName" className="sr-only">Plant Species Name</Label>
            <Input
              id="speciesName"
              name="speciesName"
              type="text"
              placeholder="E.g., Anubias nana, Java fern, Monte Carlo"
              required
            />
          </div>
